...
from AWS_Clients.ClientResponses import ClientResponseObject

# """Copied helper functions from https://github.com/capless/warrant/blob/master/warrant/aws_srp.py"""
...
def extract_deliver_details(response: ClientResponseObject) -> dict:
    delivery_details = {}
    if response.aws_response.get("CodeDeliveryDetails"):
        delivery_details["Destination"] = response.aws_response[
            "CodeDeliveryDetails"
        ].get("Destination")
        delivery_details["DeliveryMedium"] = response.aws_response[
            "CodeDeliveryDetails"
        ].get("DeliveryMedium")
    return delivery_details


def extract_tokens(response: ClientResponseObject) -> dict:
    access_token = response.aws_response["AuthenticationResult"]["AccessToken"]
    id_token = response.aws_response["AuthenticationResult"]["IdToken"]
    refresh_token = (
        response.aws_response["AuthenticationResult"].get("RefreshToken") or ""
    )
    expires_in = response.aws_response["AuthenticationResult"]["ExpiresIn"]
    return {
...
def extract_device_data(response: ClientResponseObject) -> dict:
    device_data = {}
    auth_result = response.aws_response.get("AuthenticationResult")
    if auth_result and "NewDeviceMetadata" in auth_result:
        device_metadata = auth_result["NewDeviceMetadata"]
        device_data["DeviceKey"] = device_metadata["DeviceKey"]
        device_data["DeviceGroupKey"] = device_metadata["DeviceGroupKey"]
    return device_data
    ...
    def authenticate_user(self, client=None):
        ...
        if response["ChallengeName"] == self.PASSWORD_VERIFIER_CHALLENGE:
            ...
            if tokens.get("ChallengeName") == self.NEW_PASSWORD_REQUIRED_CHALLENGE:
                return "Change password before authenticating"
